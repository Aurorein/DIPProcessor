...
void region_growing::excute_growing(){
    ...
    // 保存图像分割后的数据数组
    uint8_t *data_growing = new uint8_t[size_image];
    ...
    // 递归dfs遍历coors_(leetcode 200)
    for(int* c: coors_){
        dfs(vis, data, c[0], c[1]);
    }

    // 连通的标记为true，未连通的标记为false
    for(int i = 0; i < height; ++i) {
        for(int j = 0; j < width; ++j) {
            if(vis[i][j]){
                *(data_growing + i * line_bytes + j) = 255;
            }else{
                *(data_growing + i * line_bytes + j) = 0;
            }
        }
    }
    ...
    // 将位图数据写入磁盘
    file.write(reinterpret_cast<const char*>(data_growing), sizeof(uint8_t)*size_image);
...
void region_growing::dfs(bool** vis, uint8_t* data, int i, int j){
    int32_t width = bmp_file_.get_width();
    int32_t height = bmp_file_.get_height();
    ...
    for(int k = 0; k < 4; ++k) {
        int ii = i + coor[k];
        int jj = j + coor[k + 1];
        if(ii < 0 || ii >= height || jj < 0 || jj >= width || vis[ii][jj] || abs(*(data + ii * line_bytes + jj) - *(data + i*line_bytes + j)) > 2) {
            continue;
        }
        dfs(vis, data, ii, jj);
    }
